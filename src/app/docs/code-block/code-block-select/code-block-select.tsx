...
const NpmIcon = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    <g fill="none" fillRule="evenodd">
      <rect width="24" height="24" fill="currentColor" className="opacity-0" />
      <path
        fill="currentColor"
...
const PnpmIcon = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" className={className}>
    <path d="M30,10.75H21.251V2H30Z" fill="currentColor" />
    <path d="M20.374,10.75h-8.75V2h8.75Z" fill="currentColor" />
    <path d="M10.749,10.75H2V2h8.749Z" fill="currentColor" />
    <path d="M30,20.375H21.251v-8.75H30Z" fill="currentColor" />
...
export function CodeBlockSelect({ pnpm, yarn, npm, bun }: CodeBlockSelectProps) {
  const managers = {
    bun: { label: "bun", cmd: bun, icon: BunIcon },
    npm: { label: "npm", cmd: npm, icon: NpmIcon },
    pnpm: { label: "pnpm", cmd: pnpm, icon: PnpmIcon },
    yarn: { label: "yarn", cmd: yarn, icon: YarnIcon },
  } as const;

  type ManagerKey = keyof typeof managers;

  const availableManagers = (Object.keys(managers) as ManagerKey[]).filter(
    (m) => managers[m].cmd
  );
  const [activeManager, setActiveManager] = useState<ManagerKey>(
    availableManagers[0] || "bun"
  );
  ...
  const handleCopy = () => {
    const textToCopy = managers[activeManager].cmd;
  ...
  const ActiveIcon = managers[activeManager]?.icon || Terminal;

  return (
    <div className="w-fit rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 shadow-sm">
      {/* Code Display Area */}
      <div className="relative p-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 overflow-x-auto flex-1">
            <span className="select-none text-zinc-400 dark:text-zinc-500">$</span>
            <span className="text-zinc-800 dark:text-zinc-100 font-mono text-sm">
              {managers[activeManager]?.cmd}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div ref={dropdownRef} className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
              >
                <ActiveIcon className="h-3.5 w-3.5" />
                <span>{activeManager}</span>
                <ChevronDown
              ...
              {isOpen && (
                <div className="absolute right-0 top-full mt-1 w-32 rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-lg z-50 overflow-hidden">
                  {availableManagers.map((manager) => {
                    const Icon = managers[manager].icon;
                    return (
                      <button
                        key={manager}
                        onClick={() => {
            ...
            <button
              onClick={handleCopy}
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 transition-all hover:bg-zinc-100 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-950"
