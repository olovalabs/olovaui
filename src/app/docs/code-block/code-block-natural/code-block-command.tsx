...
export function CodeBlockCommand({ pnpm, yarn, npm, bun }: CodeBlockCommandProps) {
  const managers = {
  ...
  const availableManagers = (Object.keys(managers) as ManagerKey[]).filter(m => managers[m].cmd);
  const [activeTab, setActiveTab] = useState<ManagerKey>(availableManagers[0] || "pnpm");
  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    const textToCopy = managers[activeTab].cmd;
  ...
  return (
    <div className="w-full max-w-2xl rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-950 dark:text-zinc-50 shadow-sm">
      ...
      <div className="relative p-4">
        <div className="relative rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 font-mono text-sm">
