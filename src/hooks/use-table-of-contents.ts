...
export function useTableOfContents() {
  ...
  useEffect(() => {
    const extractHeadings = () => {
      setTimeout(() => {
        const mainContent = document.querySelector("main .prose") || document.querySelector("main");
        ...
        headings.forEach((heading, index) => {
          if (heading.parentElement !== mainContent) return;

          const clone = heading.cloneNode(true) as HTMLElement;
          // Remove buttons and hidden content (e.g. anchor links, copy buttons)
          clone.querySelectorAll("button, .sr-only, .visually-hidden").forEach((el) => el.remove());
