...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose }) => {
  ...
  const filteredItems = React.useMemo(() => {
    if (query.trim()) {
      ...
      return [...componentItems, ...otherItems];
    }
  }, [query, allItems]);
  ...
  // Reset selected index when query changes (render-time update)
                ...
                <div className="py-2">
                  {filteredItems.map((item, index) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={onClose}
                      className={`flex items-center gap-3 px-4 py-3 hover:bg-white/50 dark:hover:bg-zinc-900/50 transition-colors ${index === selectedIndex
