...
export function SeoMetricsMonitor() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => setIsVisible(shouldShow), 0);
    ...
    <div className="fixed bottom-4 right-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 shadow-lg max-w-sm z-50">
      ...
      <div className="mt-3 pt-2 border-t border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex space-x-2 mt-1">
          <div className={`w-2 h-2 rounded-full ${metrics.largestContentfulPaint <= 2500 ? 'bg-green-500' :
              metrics.largestContentfulPaint <= 4000 ? 'bg-yellow-500' : 'bg-red-500'
            }`} title="LCP" />
          <div className={`w-2 h-2 rounded-full ${metrics.firstInputDelay <= 100 ? 'bg-green-500' :
              metrics.firstInputDelay <= 300 ? 'bg-yellow-500' : 'bg-red-500'
            }`} title="FID" />
          <div className={`w-2 h-2 rounded-full ${metrics.cumulativeLayoutShift <= 0.1 ? 'bg-green-500' :
...
export default SeoMetricsMonitor